...
func (p Parser) commandType() (uint8, error) {
	...
	if regexp.MustCompile(`^(neg|not|add|sub|and|or|eq|gt|lt)$`).MatchString(cur) {
		...
		return C_PUSH, nil
	} else if regexp.MustCompile(`^label\s+`).MatchString(cur) {
		return C_LABEL, nil
	} else if regexp.MustCompile(`^goto\s+`).MatchString(cur) {
		return C_GOTO, nil
	} else if regexp.MustCompile(`^if-goto\s+`).MatchString(cur) {
		return C_IF, nil
	} else if regexp.MustCompile(`^function\s+`).MatchString(cur) {
		return C_FUNCTION, nil
	} else if regexp.MustCompile(`^return$`).MatchString(cur) {
		return C_RETURN, nil
